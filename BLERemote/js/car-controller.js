// Car Controller for Infinity Learning Space
class CarController {
    constructor() {
 ...
 setupEventListeners() {
    ...
    controlButtons.forEach(button => {
        ...
        const voiceButton = document.querySelector('#car-screen #voice-btn');
        if (voiceButton) {
        ...
        const tiltToggle = document.querySelector('#car-screen #tilt-toggle');
        if (tiltToggle) {
		...
		const autoButtons = document.querySelectorAll('#car-screen .auto-btn');
        ...
        const stopAutoButton = document.querySelector('#car-screen #stop-auto')
        if (stopAutoButton) {
        ...
        const tabButtons = document.querySelectorAll('#car-screen .tab-btn');
    ...
    switchTab(tabId) {
        // Update tab buttons
        const tabButtons = document.querySelectorAll('#car-screen .control-tabs .tab-buttons .tab-btn');
    ...
    sendMovementCommand(direction) {
        const command = getCommand('smartcar', 'movement', direction);
        if (command) {
    ...
    setSpeed(speed) {
        ...
        const command = getCommand('smartcar', 'speed', speed);
        if (command) {
            bluetoothHandler.sendData(command);
    ...
    activateAutoMode(mode) {
        ...
        const command = getCommand('smartcar', 'autoModes', mode);
        if (command) {
            bluetoothHandler.sendData(command);
    ...
    stopAutoMode() {
        if (this.activeAutoMode) {
            const command = getCommand('smartcar', 'autoModes', 'stop');
            if (command) {
                bluetoothHandler.sendData(command);
...
sendStopCommandOnRelease() {
    ...
    startVoiceRecognition() {
        ...
        recognition.onresult = (event) => {
            ...
            // Parse voice command
            const command = parseVoiceCommand('smartcar', transcript);
            if (command) {
                bluetoothHandler.sendData(command);
	...
	logToConsole(message) {
		const consoles = [
			document.getElementById('car-console')
		];
...
// Create car controller when page loads
